...
install_rabbitmq_ha() {
  source ./config/.env
  RABBITMQ_USERNAME=${RABBITMQ_USERNAME:?"Must provide RABBITMQ_USERNAME in ./config/.env file. e.g. user"}
  RABBITMQ_PASSWORD=${RABBITMQ_PASSWORD:?"Must provide RABBITMQ_PASSWORD in ./config/.env file. e.g. password"}
  RABBITMQ_ERLANG_COOKIE=${RABBITMQ_ERLANG_COOKIE:?"Must provide RABBITMQ_ERLANG_COOKIE in ./config/.env file. e.g. SECRET"}
  RABBITMQ_PUBLIC_DOMAIN=${RABBITMQ_PUBLIC_DOMAIN:?"Must provide RABBITMQ_PUBLIC_DOMAIN in ./config/.env file. e.g. rmq.example.com"}
  RABBITMQ_TLS_SECRETNAME=${RABBITMQ_TLS_SECRETNAME:?"Must provide RABBITMQ_TLS_SECRETNAME in ./config/.env file. e.g. rmq-example-tls"}
...
echo "Do you wish to install rabbitmq-ha? enter 1 or 2:"
select yn in "Yes" "No"; do
    case $yn in
        Yes )
            install_rabbitmq_ha
